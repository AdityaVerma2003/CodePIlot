...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = `
Analyze this ${language || 'code'} and provide helpful suggestions for improvement:
...
Please provide 3-5 specific, actionable suggestions to improve this code. Focus on:
...
Format each suggestion as a concise bullet point starting with an emoji.
`;
    ...
    const response = await client.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert code reviewer and programming mentor. Provide concise, actionable suggestions to improve code quality, performance, and best practices."
        },
        {
          role: "user",
          content: prompt
    ...
    const suggestions = response.choices[0]?.message?.content
      ?.split('\n')
      .filter(line => line.trim().startsWith('•') || line.trim().startsWith('-') || /^[0-9]/.test(line.trim()) || /^[🔥💡⚡🚀✨🔧📊🐛]/u.test(line.trim()))
